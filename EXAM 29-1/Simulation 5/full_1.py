...
def func3(S):
    ...
    for word in words:
           if ord(word[0].lower())==ord('a') or ord(word[0].lower())==ord('e') or ord(word[0].lower())==ord('u') or ord(word[0].lower())==ord('o') or ord(word[0].lower())==ord('i'):
            res.append(word)
    return sorted(res)
...
def func4(input_filepath, output_filepath):
    letter_dict , word_count = operator(input_filepath)
    sorted_dict = dict(sorted(letter_dict.items(), key=lambda item: item[0], reverse=True))
    with open(output_filepath,'w') as fout:
        for k,v in sorted_dict.items():
            fout.write(k + ': ' + str(v) + '\n')
    return word_count

def word_reader(input_filepath):
    words=[]
...
def operator(input_filepath):
    visited=[]
    list_words=word_reader(input_filepath)
    letter_dict={}
    word_count=len(list_words)
    for word in list_words:
        letter_dict[word[-1].lower()]=0
    ...
    for word in list_words:
        if word.lower() in visited:
            ...
            letter_dict[word[-1].lower()]+=1

    return letter_dict , word_count
    ...
    for row in range(len(image)):
        for col in range(len(image[0])):
            if image[row][col]!=black:
                c = image[row][col]
                if row+1<len(image) and col+1<len(image[0]):
                    if image[row][col+1]== image[row+1][col]== image[row+1][col+1]== c:
                        image[row][col]=black
                        image[row][col+1]=black
                        image[row+1][col]=black
                        image[row+1][col+1]=black
                        number_2x2+=1
...
def helper(T,level=0):
    if T is None:
        ...
        T.value = '*' * level
        level+=1
        left_node=helper(T.left,level)
        right_node=helper(T.right,level)
